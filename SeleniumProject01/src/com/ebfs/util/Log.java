...
public class Log {
	...
	private static final String relResulXLtDir="./ResultExcel/";
	...
	public static void createResultXL(String name) {
		// Used to created the xls log file
		failCounter = 0;
		SimpleDateFormat date = new SimpleDateFormat("EEE_MMM_d yy_hh_mm_ss_a");
		
		String formatedDate = date.format(new Date());
		
		xlFileName=relResulXLtDir + name +" - " + formatedDate + ".xls";
		fileXLS = new File(xlFileName);
		
		try {
			fileXLS.createNewFile();
			WritableWorkbook workbook = Workbook.createWorkbook(fileXLS);
			WritableSheet sheet = workbook.createSheet("TestResult", 0); 
		...
		*for the column the first argument is the column number the second is the height which is 1 to 1 
		...
		try {
			...
			label = new Label(1, 0, "Test Steps",fontColor); 
			workSheet.addCell(label);
			
			label = new Label(2, 0, "Description",fontColor); 
			workSheet.addCell(label);
	 ...
	 */
	public static void write(String description, String stepName ,Boolean Passed) 
	{
		...
		Label label = new Label(1, num, description, defaultCell);
		workSheet.addCell(label);
		
		label = new Label(2, num, stepName, defaultCell);
		workSheet.addCell(label);
		
		//fontColor added to create different colors for pass and fail
		label = new Label(3, num, PF, fontColor);
		workSheet.addCell(label);
...
//		}
	 ...
	 * @throws WriteException
	 */
	public static String close() throws IOException, WriteException
	{
	...
	public static void main(String[] args){
		try{
		Log log = new Log("TestCase","C:\\Rabbani\\");
		System.out.println("File Created");
		log.write("Description - 1", "Test Steps - 1", true);
		log.write("Description - 3", "Test Steps - 3 ", false);
		log.takeSnapshot("C:\\Rabbani", "test Image");
		
		log.write("Description - 4", "Test Steps - 4 ", true);
		log.close();
